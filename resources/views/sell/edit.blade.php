        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <!-- /.card-header -->
                    <form class="form-horizontal" method="post" action="{{ url('/sell/update_submit') }}">
                        @csrf
                        <input type="hidden"  name="id" value="{{$sell->id}}">
                        <input type="hidden"  name="payment_id" value="{{ $sell->payment_id}}">
                        <div class="card-body">
                        <div class="form-group row">
                                <label  class="col-sm-2 col-form-label">Order Date</label>
                                <div class="col-sm-10">
                                    <input type="date"  class="form-control @error('sell_date') is-invalid @enderror" name="sell_date" placeholder="sell_date" value="{{ old('sell_date',$sell->bill_date) }}" />
                            ...
                            <div class="form-group row" id="old_client_select">
                                ...
                                <div class="col-sm-10">
                                      ...
                                      <option value="{{$val->id}}" @if ($val->id == old('Client_name',$sell->client_id)) selected="selected"  @endif>{{$val->client_name}} (GST: {{$val->client_gst_no}})</option>
                                      @endforeach
                                      </select>
                                    @error('Client_name')
                    ...
                    <div class="row">
                        <table class="table" id="productTable">
                            <thead>
                                ...
                                @if(old('productName',$product))
                                    <tbody>
                                        @foreach(old('productName',$product) as $key=>$val)
                                        @php $key++; @endphp
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div>
                                    <input type="text" class="form-control" id="subTotal" value="{{ old('subTotal',$sell->sum_amount) }}" name="subTotal"  readonly />
                                </div>
                            </div>
                            <!--/form-group-->
                            <div class="form-group">
                                ...
                                <div>
                                    <input type="text" class="form-control" id="gstTotal" value="{{ old('gstTotal',$sell->Gst_Total) }}" name="gstTotal" readonly />
                                </div>
                            </div>
                            <!--/form-group-->
                            <div class="form-group">
                                ...
                                <div>
                                    <input type="text" class="form-control" id="totalAmount" value="{{ old('totalAmount',$sell->totel_amount) }}" name="totalAmount" readonly />
                                </div>
                            </div>
                            <!--/form-group-->
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div>
                                    <input type="text" class="form-control" id="paid" value="{{ old('paid',$sell->Paid_amount) }}" name="paid" autocomplete="off" onkeyup="paidAmount()" />
                                </div>
                            </div>
                            <!--/form-group-->
                            <div class="form-group">
                                ...
                                <div>
                                    <input type="text" class="form-control" value="{{ old('due',$sell->Due_Total) }}" id="due" name="due" readonly />
                                </div>
                            </div>
                            <!--/form-group-->
                            <div class="form-group">
                                ...
                                <div>
                                    <select class="form-control" name="paymentType" id="paymentType">
                                        <option value="">~~SELECT~~</option>
                                        <option value="cheque" {{ (old("paymentType",$sell->Payment_type) == "cheque" ? "selected":"") }}>Cheque</option>
                                        <option value="cash" {{ (old("paymentType",$sell->Payment_type) == "cash" ? "selected":"") }}>Cash</option>
                                        <option value="credit_Card" {{ (old("paymentType",$sell->Payment_type) == "credit_Card" ? "selected":"") }}>Credit Card</option>
                                    </select>
                                </div>
                            </div>
                            <!--/form-group-->
                            <div class="form-group">
                                ...
                                <div>
                                    <select class="form-control" name="paymentStatus" id="paymentStatus">
                                        <option value="">~~SELECT~~</option>
                                        <option value="Full_Payment" {{ (old("paymentStatus",$sell->Payment_status) == "Full_Payment" ? "selected":"") }}>Full Payment</option>
                                        <option value="Advance_Payment" {{ (old("paymentStatus",$sell->Payment_status) == "Advance_Payment" ? "selected":"") }}>Advance Payment</option>
                                        <option value="No_Payment" {{ (old("paymentStatus",$sell->Payment_status) == "No_Payment" ? "selected":"") }}>No Payment</option>
                                    </select>
                                </div>
                            </div>
                            <!--/form-group-->
